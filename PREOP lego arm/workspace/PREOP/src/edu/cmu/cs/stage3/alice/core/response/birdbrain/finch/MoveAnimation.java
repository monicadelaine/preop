...
public class MoveAnimation extends DirectionAmountTransformAnimation {
	...
	public class RuntimeMoveAnimation extends RuntimeDirectionAmountTransformAnimation {
		...
		public void prologue( double t ) { 
			...
			if(((Finches)MoveAnimation.this.subject.getValue()).isConnected() && (edu.cmu.cs.stage3.alice.authoringtool.AuthoringTool.realPlay || !(edu.cmu.cs.stage3.alice.authoringtool.AuthoringTool.getHack().getWorld().isRunning()))){
				if(MoveAnimation.this.direction.getValue().equals(edu.cmu.cs.stage3.alice.core.Direction.FORWARD))
					((Finches)MoveAnimation.this.subject.getValue()).moveForward(((Double)MoveAnimation.this.duration.getValue()).doubleValue());
				if(MoveAnimation.this.direction.getValue().equals(edu.cmu.cs.stage3.alice.core.Direction.BACKWARD))
					((Finches)MoveAnimation.this.subject.getValue()).moveBackward(((Double)MoveAnimation.this.duration.getValue()).doubleValue());
			}
			super.prologue( t );
