...
public class AddressPropertyViewController  extends TextFieldEditablePropertyViewController{
	...
	protected void setValueFromString( String valueString ) {
		...
		Address valueAddress = new Address(valueString);
		((Runnable)factory.createItem( valueAddress )).run();
	}
		...
		return new java.awt.event.MouseAdapter() {
			public void mouseReleased( java.awt.event.MouseEvent ev ) {
				if( (ev.getX() >= 0) && (ev.getX() < ev.getComponent().getWidth()) && (ev.getY() >= 0) && (ev.getY() < ev.getComponent().getHeight()) ) {
					if( isEnabled() ) {
						AddressPropertyViewController.this.popupButton.doClick();
					}
				}
			}
		};
