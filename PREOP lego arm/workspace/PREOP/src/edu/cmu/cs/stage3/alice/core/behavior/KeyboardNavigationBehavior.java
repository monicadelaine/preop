...
public class KeyboardNavigationBehavior extends InternalResponseBehavior {
	...
	public void internalSchedule( double time, double dt ) {
		...
		if(subject.getValue() instanceof edu.cmu.cs.stage3.alice.core.irobot.Create){
			//Possibly slow down for more acuracy
			//Consider possibility of other methods occuring simultaniously
			speed.z=turning.y=0;
			if ((actions & KeyMapping.NAV_MOVEFORWARD) != 0)
				speed.z = .5;
			if ((actions & KeyMapping.NAV_MOVEBACKWARD) != 0)
				speed.z = -.5;
			if ((actions & KeyMapping.NAV_TURNRIGHT) != 0)
				turning.y = .5;
